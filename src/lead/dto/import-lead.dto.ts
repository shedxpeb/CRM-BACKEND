import { ImportHeaderMap, ImportEnumMap, ImportConfig } from '../../common/services/excel-import.service';
import { LeadSource, LeadPriority, LeadStatus, ProjectType, StructureType, RoofType, WallType, MaterialPreference } from './get-leads.dto';

export const LEAD_HEADER_MAP: ImportHeaderMap = {
  customerName: ['customer name', 'customername', 'name', 'contact name', 'contact', 'client name', 'client'],
  ...
  mobile: ['mobile', 'mobile no', 'mobile no.', 'mobile number', 'phone', 'phone number', 'contact number', 'contact no', 'contact no.', 'telephone', 'tel'],
  alternateMobile: ['alternate mobile', 'alternate phone', 'alt mobile', 'alt phone', 'secondary mobile', 'secondary phone', 'other phone', 'other mobile'],
  ...
  addressLine1: ['address', 'address 1', 'address line 1', 'address line', 'street', 'street address', 'addressline1'],
  ...
  projectTitle: ['project title', 'project name', 'project', 'title', 'requirement', 'requirement title'],
  ...
  specialRequirement: ['special requirement', 'special req', 'custom requirement', 'additional requirement'],
  ...
  assignedTo: ['assigned to', 'assignee', 'owner', 'sales rep', 'sales person', 'assigned employee'],
  remarks: ['remarks', 'comment', 'internal notes', 'internal remark'],
  nextFollowUpDate: ['follow up date', 'follow-up date', 'next follow up', 'next follow-up', 'followup date'],
};
...
export const LEAD_IMPORT_CONFIG: ImportConfig = {
  ...
  transformRow: (row: Record<string, any>) => {
    ...
    for (const numField of ['width', 'length', 'height', 'baySpacing', 'craneCapacity', 'mezzanineArea', 'mezzanineLoad', 'insulationThickness', 'annualRevenue']) {
