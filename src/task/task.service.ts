...
export class TaskService extends BaseQueryService {
  constructor(
    ...
    super(prisma, {
      ...
      filterFields: ['status', 'priority', 'category', 'linkedModule', 'assignedUserId', 'projectId'],
  ...
  async create(dto: CreateTaskDto, organizationId: string, createdById: string, createdByName: string) {
    const orgId = this.requireOrganizationId(organizationId);
    ...
    await this.logActivity(task.id, 'Created', `Task "${task.title}" created`, createdById, createdByName, orgId);

    if (dto.beforeImages && dto.beforeImages.length > 0) {
      await this.logActivity(task.id, 'Before Images Added', `${dto.beforeImages.length} before images uploaded`, createdById, createdByName, orgId);
    }
  ...
  async update(
    ...
    if (existing.status === 'Completed' || existing.status === 'Verified' || existing.status === 'Closed') {
    ...
    if (changes.length > 0) {
      await this.logActivity(id, 'Updated', `Task updated: ${changes.join(', ')}`, updatedBy, updatedByName, orgId);
    }
  ...
  async deleteTask(id: string, organizationId: string, deletedById: string, deletedByName: string) {
    ...
    await this.logActivity(id, 'Cancelled', `Task "${existing.title}" deleted`, deletedById, deletedByName, orgId);
    return { message: 'Task deleted successfully' };
  }

  async complete(id: string, dto: CompleteTaskDto, organizationId: string, userId: string, userName: string) {
    const orgId = this.requireOrganizationId(organizationId);
    const existing = await this.findById(id, orgId);
    ...
    await this.logActivity(id, 'Completed', `Task completed. ${dto.completionNotes}`, userId, userName, orgId);

    if (dto.completionProof.afterImages.length > 0) {
      await this.logActivity(id, 'After Images Added', `${dto.completionProof.afterImages.length} after images uploaded`, userId, userName, orgId);
    }
  ...
  async verify(id: string, dto: VerifyTaskDto, organizationId: string) {
    ...
    await this.logActivity(id, action, `Task ${action.toLowerCase()}${notes}`, dto.verifiedBy || '', dto.verifiedByName || '', orgId);
  ...
  async getStats(organizationId: string) {
    ...
    const [
      ...
      this.client.count({ where: { ...baseWhere, dueDate: { lt: now }, status: { in: ['Pending', 'InProgress', 'Reopened'] } } }),
      this.client.count({ where: { ...baseWhere, dueDate: { gte: todayStart, lte: todayEnd } } }),
      this.client.count({ where: { ...baseWhere, dueDate: { gte: todayStart, lte: weekEnd }, status: { in: ['Pending', 'InProgress', 'Reopened'] } } }),
      this.client.count({ where: { ...baseWhere, status: 'Completed' } }),
      this.client.count({ where: { ...baseWhere, status: 'Completed', completedAt: { gte: todayStart, lte: todayEnd } } }),
      this.client.groupBy({ by: ['priority'], where: baseWhere, _count: true }),
      this.client.groupBy({ by: ['status'], where: baseWhere, _count: true }),
      this.client.groupBy({ by: ['linkedModule'], where: { ...baseWhere, linkedModule: { not: null } }, _count: true }),
    ]);
    ...
    tasksByPriority.forEach((p: any) => { priorityMap[p.priority] = p._count; });

    const statusMap: Record<string, number> = {};
    tasksByStatus.forEach((s: any) => { statusMap[s.status] = s._count; });

    const moduleMap: Record<string, number> = {};
    tasksByModule.forEach((m: any) => { moduleMap[m.linkedModule || 'General'] = m._count; });
  ...
  async getDashboardKPIs(organizationId: string) {
    ...
    const [openTasks, overdueTasks, completedToday, pendingVerification, verifiedToday] = await Promise.all([
      this.client.count({ where: { organizationId: orgId, isDeleted: false, status: { in: ['Pending', 'InProgress', 'Reopened'] } } }),
      this.client.count({ where: { organizationId: orgId, isDeleted: false, dueDate: { lt: now }, status: { in: ['Pending', 'InProgress', 'Reopened'] } } }),
      this.client.count({ where: { organizationId: orgId, isDeleted: false, status: 'Completed', completedAt: { gte: new Date(now.getFullYear(), now.getMonth(), now.getDate()) } } }),
      this.client.count({ where: { organizationId: orgId, isDeleted: false, status: 'Completed' } }),
      this.client.count({ where: { organizationId: orgId, isDeleted: false, status: 'Verified', verifiedAt: { gte: new Date(now.getFullYear(), now.getMonth(), now.getDate()) } } }),
    ]);
  ...
  async getEmployeePerformance(organizationId: string, employeeId?: string) {
    ...
    const performance = Array.from(employeeMap.values()).map(emp => {
      ...
      emp.finalPayable = emp.baseSalary + emp.verifiedTaskIncentives + emp.bonuses - emp.advances - emp.penalties;
      return emp;
    });
  ...
  async getEmployeeSalaryLedger(organizationId: string, employeeId: string, periodStart: Date, periodEnd: Date) {
    const orgId = this.requireOrganizationId(organizationId);
    ...
    const pendingIncentives = tasks
      .filter(t => !['Verified', 'Closed'].includes(t.status))
      .reduce((sum, t) => sum + t.incentiveValue, 0);

    const bonuses = adjustments.filter(a => a.type === 'Bonus' && a.status === 'Approved').reduce((s, a) => s + a.amount, 0);
    const advances = adjustments.filter(a => a.type === 'Advance' && a.status === 'Approved').reduce((s, a) => s + a.amount, 0);
    const penalties = adjustments.filter(a => a.type === 'Penalty' && a.status === 'Approved').reduce((s, a) => s + a.amount, 0);
    const credits = adjustments.filter(a => a.type === 'Credit' && a.status === 'Approved').reduce((s, a) => s + a.amount, 0);
    const deductions = adjustments.filter(a => a.type === 'Deduction' && a.status === 'Approved').reduce((s, a) => s + a.amount, 0);
    ...
    return {
      ...
      totalPaid: advances,
      lastPaymentDate: adjustments.filter(a => a.type === 'Advance').sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0]?.createdAt,
      adjustments,
      periodStart,
      periodEnd,
    };
  ...
  async getSalaryAdjustments(query: GetSalaryAdjustmentsDto, organizationId: string) {
    const orgId = this.requireOrganizationId(organizationId);
    const { page = 1, pageSize = 25, employeeId, type, status, dateFrom, dateTo, sortBy = 'createdAt', sortOrder = 'desc' } = query;
  ...
  async createSalaryAdjustment(dto: CreateSalaryAdjustmentDto, organizationId: string, createdById: string) {
    const orgId = this.requireOrganizationId(organizationId);
  ...
  async approveSalaryAdjustment(id: string, approvedBy: string, approvedByName: string, organizationId: string) {
    const orgId = this.requireOrganizationId(organizationId);
    const existing = await this.getSalaryAdjustmentById(id, orgId);
